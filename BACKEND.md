...
Endpoints: POST/GET /api/incidents; GET /api/incidents/:id; POST /api/incidents/:id/updates; GET /api/incidents/:id/rescue-plan; GET /api/incidents/:id/rescue-plans; GET /api/incidents/:id/agent-status; GET /api/incidents/:id/agent-trace; POST /api/incidents/:id/agent-recheck; POST /api/assistant/analyze; GET /api/guides and /api/guides/:id. IDs must be UUIDs (legacy NR-UUID is accepted). schema_version=1 required for writes. Coordinates, dates, counts, enums, priority and ownership checked. Payload limit 32 KiB. Safe JSON errors. CORS exact allowlist; null file origin only in development. Per-address rate limit applies to nonduplicate protected operations; duplicate incident/update retry reuses ACK.

SQLite WAL tables retain the original incident/message structures and add `agent_jobs`, `agent_trace`, and `rescue_plans`. Incident, ACK, and durable pending job commit in one transaction; agent processing starts outside the request path. Original acknowledgement is immutable; each update has its own ACK. Server priority cannot fall below existing locked priority. Incoming timestamps are retained; server received_at is separate. Rescue planning never performs diagnosis and never represents a human responder.
...
Run tests: `node --test backend/tests/*.test.mjs` from SOURCE-CODE. Production startup does not seed mock incidents, responders, hazards, routes, or agent results.
